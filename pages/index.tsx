...
export default function Home() {
  ...
  async function startRecording() {
    ...
    await navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      ...
      media.onstop = async function () {
        ...
        const dataURL = await fetchFileURL(audioBlob);
        console.log(dataURL);
        // const audioFile = new File([audioBlob], 'recording.flac', {
        //   type: 'audio/flac',
        // });
        // const data = await fetchFileURL(audioFile);
        // console.log(data);
  ...
  async function fetchFileURL(audioBlob: any) {
    const reader = new FileReader();
  
    return new Promise((resolve, reject) => {
      reader.onloadend = () => {
        const fileURL = URL.createObjectURL(audioBlob); // Tạo URL từ đối tượng File
        resolve(fileURL);
      };
  
      reader.onerror = reject;
  
      reader.readAsDataURL(audioBlob); // Đọc dữ liệu dưới dạng Data URL
    });
  }
